//*********************************************************************************************
//* Name: node-uploadPhotoToNode.js
//* Description: Katapult Pro API v3 — Upload a JPEG photo and associate it to a node.
//* Disclaimer: Katapult Engineering assumes no responsibility or liability for any errors
//*             or omissions in the content of this Katapult API Example.
//* Setup: npm install axios
//*********************************************************************************************
const axios = require('axios');
const fs = require('fs');

const URL_PATH = 'https://katapultpro.com/api/v3';
const API_KEY = '<<YOUR_API_KEY>>';

// Path placeholders — fill in with your own ids.
const JOB_ID = '<<YOUR_JOB_ID>>';
const NODE_ID = '<<YOUR_NODE_ID>>';

// Optional query: how to associate the photo. 'true' (default) or 'main'.
const ASSOCIATION_VALUE = 'main';

async function run() {
  try {
    // Send the raw JPEG bytes from a local file with Content-Type: image/jpeg.
    const imageBytes = fs.readFileSync('sample.jpg');
    const response = await axios({
      method: 'post',
      url: `${URL_PATH}/jobs/${JOB_ID}/nodes/${NODE_ID}/photos?api_key=${API_KEY}&association_value=${ASSOCIATION_VALUE}`,
      headers: { 'Content-Type': 'image/jpeg' },
      data: imageBytes,
    });
    console.log('Status:', response.status);
    console.log(JSON.stringify(response.data, null, 2));
  } catch (error) {
    console.error('Error', error.response?.status, JSON.stringify(error.response?.data));
  }
}

run();
